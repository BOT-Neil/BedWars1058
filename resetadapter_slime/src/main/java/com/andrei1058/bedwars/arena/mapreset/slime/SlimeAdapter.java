...
public class SlimeAdapter extends RestoreAdapter {
  ...
  public void onEnable(IArena a) {
    if (api.getVersionSupport().getMainLevel().equalsIgnoreCase(a.getWorldName())) {
      if (!(api.getServerType() == ServerType.BUNGEE && api.getArenaUtil().getGamesBeforeRestart() == 1)) {
        ...
        getOwner().getLogger().log(Level.SEVERE,
            "Cannot use level-name as arenas. Automatically creating a new void map for level-name.");
    ...
    Bukkit.getScheduler().runTaskAsynchronously(getOwner(), () -> {
      ...
      try {
        ...
        Bukkit.getScheduler().runTask(getOwner(), () -> slime.loadWorld(finalWorld));
      } catch (Exception ex) {
    ...
    if (api.getServerType() == ServerType.BUNGEE) {
      if (api.getArenaUtil().getGamesBeforeRestart() == 0) {
        if (api.getArenaUtil().getArenas().size() == 1
          ...
          getOwner().getLogger().info("Dispatching command: "
              + api.getConfigs().getMainConfig().getString(ConfigPath.GENERAL_CONFIGURATION_BUNGEE_OPTION_RESTART_CMD));
        ...
        Bukkit.getScheduler().runTask(getOwner(), () -> {
          ...
          if (api.getArenaUtil().canAutoScale(a.getArenaName())) {
            Bukkit.getScheduler().runTaskLater(getOwner(), () -> api.getArenaUtil().loadArena(a.getArenaName(), null),
                80L);
          }
        });
      }
      ...
      try {
        if (sLoader.worldExists(s.getWorldName())) {
          ...
          if (new File(Bukkit.getWorldContainer(), s.getWorldName() + "/level.dat").exists()) {
            Bukkit.getScheduler().runTask(getOwner(),
                () -> s.getPlayer()
                    .sendMessage(ChatColor.GREEN + "Importing world to the SlimeWorldManager container."));
            slime.importWorld(new File(Bukkit.getWorldContainer(), s.getWorldName()), s.getWorldName().toLowerCase(),
                sLoader);
            world = slime.loadWorld(sLoader, s.getWorldName(), false, spm);
          } else {
            Bukkit.getScheduler().runTask(getOwner(),
                () -> s.getPlayer().sendMessage(ChatColor.GREEN + "Creating anew void map."));
        ...
        Bukkit.getScheduler().runTask(getOwner(), () -> {
          ...
          | IllegalArgumentException | WorldAlreadyExistsException | InvalidWorldException | WorldLoadedException
  ...
  public void onLobbyRemoval(IArena a) {
    ...
    Bukkit.getScheduler().runTask(getOwner(), () -> {
      ...
      Bukkit.getScheduler().runTaskLater(getOwner(), () -> loc1.getWorld().getEntities().forEach(e -> {
  ...
  public void cloneArena(String name1, String name2) {
    Bukkit.getScheduler().runTaskAsynchronously(getOwner(), () -> {
      ...
      SlimeWorld world;
      try {
        world = slime.loadWorld(slime.getLoader("file"), name1, true, spm);
        world.clone(name2, slime.getLoader("file"));

      } catch (UnknownWorldException | CorruptedWorldException | NewerFormatException | WorldLockedException
          | IOException | WorldAlreadyExistsException e) {
        // TODO Auto-generated catch block
        e.printStackTrace();
      }
    });
                  ...
                  if (!fl.getName().equals(name)) {
                    if (!fl.renameTo(new File(dir, name + ".yml"))) {
                      getOwner().getLogger().log(Level.WARNING,
                          "Could not rename " + fl.getName() + ".yml to " + name + ".yml");
                    }
                  }
    ...
    if (!level.exists()) {
      if (level.createNewFile()) {
        ...
        if (regions.exists() && Objects.requireNonNull(regions.list()).length > 0) {
          if (Arrays.stream(Objects.requireNonNull(regions.list())).filter(p -> p.endsWith(".mca"))
              .toArray().length > 0) {
            File region = new File(Bukkit.getWorldContainer(), world + "/"
                + Arrays.stream(Objects.requireNonNull(regions.list())).filter(p -> p.endsWith(".mca")).toArray()[0]);
